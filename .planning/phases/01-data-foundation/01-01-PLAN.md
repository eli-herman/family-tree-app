...
Keep the existing RelationshipType enum as-is (it includes 'sibling' which is fine to store for the Herman family data).
  </action>
...
- Only parent/child and spouse relationships
- Siblings can be stored as 'sibling' for simplicity (existing type supports it)
...
2. Check direct relationships first (parent, child, spouse, sibling)
3. One-hop for in-laws: if target is sibling's spouse, return "Brother-in-law" or "Sister-in-law" based on gender
4. Two-hop for grandparents: if target is parent's parent, return "Grandma" or "Grandpa" based on gender
5. Default to 'Family' for unrecognized relationships
