...
Replace `mockFamilyMembers` with `members` in all filter operations:
```typescript
...
// ... same pattern for parents and children
```

**Note:** The filtering logic stays the same, just uses `members` from store instead of `mockFamilyMembers`.
